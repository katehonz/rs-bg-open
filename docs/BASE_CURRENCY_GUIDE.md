...
От версия 0.1.0+ формулярите за счетоводни записи **динамично зареждат** активните валути от базата данни, вместо да използват хардкоден списък.

#### Засегнати страници

**Journal Entries (Счетоводни записи)**
- Локация: `http://localhost/accounting/entries`
- Файл: `frontend/src/pages/JournalEntry.jsx`

**VAT Entry (ДДС запис)**
- Локация: `http://localhost/accounting/vat-entry`
- Файл: `frontend/src/pages/VATEntry.jsx`

#### Как работи

1. **При зареждане на страницата** се извиква GraphQL заявка:
...
2. **Филтрира се само активни валути** (без BGN, която е винаги показана):
...
3. **Зарежда се и обменният курс** за всяка валута:
...
4. **Dropdown менюто се генерира динамично**:
...
5. **При промяна на валута** се взима актуалният курс:

```javascript
const handleCurrencyChange = async (lineIndex, currencyCode) => {
  const rate = currencyCode === 'BGN' ? 1 : (exchangeRates[currencyCode] || 1);
  // ... приложи курса към реда
};
```

#### Fallback механизъм

Ако API не успее да зареди валути, се използва fallback списък:
...
✅ **Синхронизация с Currency Management**
- Активирате валута в `/currencies` → веднага се показва в записите
- Деактивирате валута → автоматично изчезва от dropdown

✅ **Актуални обменни курсове**
- Използват се курсове от БНБ/ECB
- Не са хардкодени стойности

✅ **Лесно добавяне на нови валути**
- Добавете валута чрез миграция
- Активирайте я в UI
- Веднага работи във всички формуляри

✅ **Правилно показване в справки**
- Само активни валути се показват
- Справките виждат данни за всички избрани валути

#### Пример за използване

**Стъпка 1:** Отидете на `/currencies` и активирайте валута TRY (Турска лира)

**Стъпка 2:** Отидете на `/accounting/entries` или `/accounting/vat-entry`

**Стъпка 3:** В dropdown за валута вече виждате TRY

**Стъпка 4:** Изберете TRY → автоматично се зарежда актуалният курс от БНБ

**Стъпка 5:** Създайте запис в TRY → данните се показват правилно в справките
