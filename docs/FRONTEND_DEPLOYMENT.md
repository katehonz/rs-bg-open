...
### Метод 1: Caddy (Препоръчителен - използва се в production)

**Автоматично SSL с Let's Encrypt + Docker интеграция**

Production deployment използва Caddy контейнер в `/path/to/caddy-proxy/`:
...
rust.cyberbuch.org {
    ...
    # GraphQL API
    handle /graphql {
        reverse_proxy accounting_service:8080
    }
    ...
    handle /assets/* {
        reverse_proxy accounting_frontend:3000
        header Cache-Control "public, max-age=31536000, immutable"
    }

    # Frontend (no cache for HTML)
    handle {
        reverse_proxy accounting_frontend:3000
    ...
    log {
        output file /var/log/caddy/rust.cyberbuch.org.log
        format json
    }
}
```
...
**Предимства:**
- ✅ Автоматично SSL от Let's Encrypt
- ✅ Автоматично обновяване на сертификати
- ✅ HTTP/3 поддръжка
- ✅ Docker network интеграция
- ✅ Оптимален cache control
...
const client = new ApolloClient({
  uri: process.env.NODE_ENV === 'production'
    ? '/graphql'  // За production (през reverse proxy - Caddy/Nginx)
...
3. Активирайте gzip/brotli компресия в reverse proxy (Caddy/Nginx)
