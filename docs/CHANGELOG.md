...
## [2025-11-05] - Фиксирани валутни курсове и настройки на фирми

### 🎯 Добавено
- **Фиксиран EUR/BGN курс** на 1.95583 (Bulgarian currency board) - никога не се променя от API
- **Настройки на валути за фирми**: Всяка фирма сега може да избере:
  - Основна валута (BGN, EUR, USD, и др.)
  - Провайдер за валутни курсове (БНБ, ЕЦБ, Ръчно, API)
- **Legacy EU валути** с фиксирани EUR курсове (HRK, LTL, LVL, EEK, SKK, CYP, MTL, SIT)
- **FixedRatesService** за управление на фиксирани валутни курсове
- Нова документация: `FIXED_EXCHANGE_RATES.md`, `CHANGELOG-COMPANY-CURRENCIES.md`

### 🔄 Променено
- **BNB Service**: Сега пропуска EUR при обновяване на курсове (фиксиран курс)
- **ECB Service**: Сега пропуска EUR/BGN конверсия (фиксиран курс)
- **Admin Companies форма**: Добавени dropdown за избор на валута и провайдер
- **База данни**: Добавена `preferred_rate_provider` колона в companies таблица

### ✅ Поправено
- EUR/BGN курсът сега винаги е 1.95583 (преди беше грешен 1.9572 от БНБ API)
- Фиксираният курс никога няма да бъде презаписан от автоматични API актуализации

Вижте [CHANGELOG-COMPANY-CURRENCIES.md](./CHANGELOG-COMPANY-CURRENCIES.md) за детайлни промени.
...
- **Динамично зареждане на валути в счетоводни записи**
  - Премахнати хардкодени списъци с валути в JournalEntry и VATEntry форми
  - Автоматично зареждане на активни валути от база данни чрез GraphQL
  - Динамично зареждане на актуални обменни курсове от БНБ/ECB
  - Автоматична синхронизация с Currency Management модула
  - Активиране на валута в `/currencies` → веднага се показва в dropdown менютата
  - Fallback механизъм към 3 основни валути при проблеми с API
  - 📖 Документация: `CURRENCY_DYNAMIC_LOADING.md`
