...
export default function BankStatementsReview({ companyId, bankProfiles, accounts }) {
  ...
  }, [bankProfiles, selectedProfileId], dateFilter);
  ...
  useEffect(() => {
    const loadEntries = async () => {
      ...
      const missingIds = entryIds.filter((id) => !entriesCache[id], dateFilter);
  ...
  }, [selectedImport], dateFilter);

  const entryDetails = useMemo(() => {
    return entryIds.map((id) => entriesCache[id] ?? { missing: true, id });
  }, [entryIds, entriesCache]);
