...
const EXPORT_MONTHLY_STATS_MUTATION = `
  mutation ExportMonthlyStats($companyId: Int!, $fromYear: Int!, $fromMonth: Int!, $toYear: Int!, $toMonth: Int!, $format: String!) {
    exportMonthlyStats(
      input: {
        companyId: $companyId
        fromYear: $fromYear
...
export default function MonthlyStats() {
  ...
  const handleExport = async (format) => {
    ...
    try {
      ...
      setError(`Грешка при експортиране на ${format} файла.`);
    } finally {
      setExporting(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          ...
          >
            {exporting ? 'Експортиране...' : '📄 Експорт ODT (LibreOffice)'}
          </button>
        </div>
      </div>
