...
const CREATE_COMPANY = gql`
  mutation CreateCompany($input: AdminCreateCompanyInput!) {
...
const UPDATE_COMPANY = gql`
  mutation UpdateCompany($id: Int!, $input: AdminUpdateCompanyInput!) {
...
const CompanyForm = ({ open, handleClose, company, onSubmit, isEdit = false, currencies }) => {
  ...
  useEffect(() => {
    if (company && isEdit) {
      setFormData({
        ...
        preferredRateProvider: company.preferredRateProvider || 'BNB',
      ...
      setFormData({
        ...
        managerEgn: '',
        authorizedPersonEgn: '',
        baseCurrencyId: currencies && currencies.length > 0 ? currencies.find(c => c.code === 'BGN')?.id || null : null,
        preferredRateProvider: 'BNB',
        isActive: true
      });
    }
  }, [company, isEdit, open, currencies]);
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={6}>
            <FormControl fullWidth margin="dense">
              <InputLabel>Основна валута</InputLabel>
              <Select
                value={formData.baseCurrencyId || ''}
                label="Основна валута"
                onChange={(e) => setFormData({ ...formData, baseCurrencyId: e.target.value || null })}
              >
                <MenuItem value="">
                  <em>Не е избрана</em>
                </MenuItem>
          ...
          <Grid item xs={6}>
            <FormControl fullWidth margin="dense">
              ...
              <Select
                value={formData.preferredRateProvider}
                label="Провайдер за курсове"
                onChange={(e) => setFormData({ ...formData, preferredRateProvider: e.target.value })}
              >
