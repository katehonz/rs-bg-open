...
impl AddressParser {
    ...
    async fn parse_with_mistral(
        ...
        let prompt = format!(
            "You are an expert at parsing Bulgarian company addresses. Extract streetName, city, postalCode, country from: {}. Return only valid JSON.",
            address
        );
        let body = json!({
            "model": self.mistral_model,
            "messages": [
                {"role": "system", "content": "Extract Bulgarian address components as JSON with fields streetName, city, postalCode, country."},
...
fn parse_with_regex(address: &str) -> AddressComponents {
    ...
    let street_regex =
        Regex::new(r"(?i)(ул\\.?|улица|бул\\.?|булевард|пл\\.?|площад)\\s*([^,\n]+)").unwrap();
    if let Some(captures) = street_regex.captures(address) {
        ...
        components.street_name = Some(format!("{} {}", prefix.trim(), rest));
    }
