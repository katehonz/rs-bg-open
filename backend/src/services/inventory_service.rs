...
use crate::entities::{
    account, average_cost_correction, entry_line, inventory_balance, inventory_movement, journal_entry,
...
impl InventoryService {
    ...
    pub async fn get_quantity_turnover(
        ...
        // Get all movements before from_date for opening balance
        let opening_movements = InventoryMovement::find()
            .filter(inventory_movement::Column::CompanyId.eq(company_id))
            .filter(inventory_movement::Column::MovementDate.lt(from_date))
            .order_by_asc(inventory_movement::Column::AccountId)
            .order_by_asc(inventory_movement::Column::MovementDate)
            .all(db)
            .await?;

        // Get movements within the period
        let period_movements = InventoryMovement::find()
            .filter(inventory_movement::Column::CompanyId.eq(company_id))
        ...
        for mov in opening_movements.iter().chain(period_movements.iter()) {
            if !account_ids.contains(&mov.account_id) {
                account_ids.push(mov.account_id);
            }
        }

        // Load account information
        let accounts = account::Entity::find()
            .filter(account::Column::Id.is_in(account_ids.clone()))
            .all(db)
            .await?;

        let accounts_map: HashMap<i32, (String, String)> = accounts
            .into_iter()
            .map(|acc| (acc.id, (acc.code, acc.name)))
            .collect();

        let mut rows: HashMap<i32, QuantityTurnoverRow> = HashMap::new();

        // Calculate opening balances from movements before period
        for movement in opening_movements {
            let (code, name) = accounts_map
                .get(&movement.account_id)
                .cloned()
                .unwrap_or_else(|| (movement.account_id.to_string(), "Unknown".to_string()));
            ...
            // Use the balance_after fields from the last movement before period
            row.opening_quantity = movement.balance_after_quantity;
            row.opening_amount = movement.balance_after_amount;
        }

        // Process period movements
        for movement in period_movements {
            let (code, name) = accounts_map
                .get(&movement.account_id)
                .cloned()
                .unwrap_or_else(|| (movement.account_id.to_string(), "Unknown".to_string()));
        ...
        for row in rows.values_mut() {
            row.closing_quantity = row.opening_quantity + row.receipt_quantity - row.issue_quantity;
            row.closing_amount = row.opening_amount + row.receipt_amount - row.issue_amount;
        }
