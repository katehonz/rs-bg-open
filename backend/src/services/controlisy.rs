            ...
            match import_document_type {
                "purchase" => {
                    ...
                    if document.vat_amount_bgn > Decimal::ZERO {
                        (Some("1"), None) // Получени доставки с право на пълен данъчен кредит
                    } else {
                        (Some("3"), None) // Получени доставки без ДДС
                    }
                }
                "sale" => {
                    // This is a sales document
                    if document.vat_amount_bgn > Decimal::ZERO {
                        (None, Some("1")) // Облагаеми доставки с 20%
                    } else {
                        (None, Some("8")) // Освободени доставки
                    }
                }
                _ => {
                    // Default to purchase
