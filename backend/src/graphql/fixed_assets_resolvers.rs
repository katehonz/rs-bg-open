...
use crate::entities::{
    asset_value_adjustment, depreciation_journal, fixed_asset, fixed_asset_category,
    AssetValueAdjustment, DepreciationJournal, FixedAsset, FixedAssetCategory,
};
use crate::services::asset_adjustment_service::AssetAdjustmentService;
...
#[derive(InputObject)]
pub struct ApplyImprovementInput {
    pub asset_id: i32,
    pub adjustment_date: NaiveDate,
    pub improvement_amount: Decimal,
    pub reason: String,
...
#[derive(InputObject)]
pub struct ApplyImpairmentInput {
    pub asset_id: i32,
    pub adjustment_date: NaiveDate,
    pub impairment_amount: Decimal,
    pub reason: String,
    pub document_number: Option<String>,
}
...
#[derive(SimpleObject)]
pub struct AssetAdjustmentResult {
    pub success: bool,
    pub adjustment_id: i32,
    pub journal_entry_id: Option<i32>,
    pub depreciation_periods_calculated: i32,
    pub message: String,
}

#[derive(SimpleObject)]
pub struct AssetAdjustmentWithAsset {
    #[graphql(flatten)]
    pub adjustment: asset_value_adjustment::Model,
    pub asset: Option<fixed_asset::Model>,
}
...
impl FixedAssetsQuery {
    ...
    /// Get asset value adjustments for a fixed asset
    async fn asset_adjustments(
        &self,
        ctx: &Context<'_>,
        asset_id: i32,
    ) -> FieldResult<Vec<AssetAdjustmentWithAsset>> {
        let db = ctx.data::<Arc<DatabaseConnection>>()?;

        let adjustments = AssetValueAdjustment::find()
        ...
        for adjustment in adjustments {
            let asset = FixedAsset::find_by_id(adjustment.fixed_asset_id)
                .one(db.as_ref())
                .await?;

            result.push(AssetAdjustmentWithAsset { adjustment, asset });
        }

        Ok(result)
    }

    /// Get asset value adjustments for a company
    async fn company_asset_adjustments(
        &self,
        ctx: &Context<'_>,
        company_id: i32,
        ...
        let mut query = AssetValueAdjustment::find()
        ...
        if let Some(adj_type) = adjustment_type {
            query = query.filter(asset_value_adjustment::Column::AdjustmentType.eq(adj_type));
        }
...
impl FixedAssetsMutation {
    ...
    /// Apply an improvement (value increase) to a fixed asset
    async fn apply_asset_improvement(
        &self,
        ctx: &Context<'_>,
        input: ApplyImprovementInput,
        user_id: i32,
    ) -> FieldResult<AssetAdjustmentResult> {
        let db = ctx.data::<Arc<DatabaseConnection>>()?;
        let service = AssetAdjustmentService::new();
        ...
        let result = service
            .apply_improvement(
                ...
                input.improvement_amount,
                input.reason,
                input.document_number,
                input.new_accounting_depreciation_rate,
                input.new_tax_depreciation_rate,
                input.new_accounting_useful_life,
                input.new_tax_useful_life,
                user_id,
            )
            .await
            .map_err(|e| format!("Failed to apply improvement: {}", e))?;

        Ok(AssetAdjustmentResult {
            success: true,
            adjustment_id: result.adjustment_id,
            journal_entry_id: result.journal_entry_id,
            depreciation_periods_calculated: result.depreciation_periods_calculated.len() as i32,
            message: result.message,
        })
    }

    /// Apply an impairment (value decrease) to a fixed asset
    async fn apply_asset_impairment(
        &self,
        ctx: &Context<'_>,
        input: ApplyImpairmentInput,
        user_id: i32,
    ) -> FieldResult<AssetAdjustmentResult> {
        let db = ctx.data::<Arc<DatabaseConnection>>()?;
        let service = AssetAdjustmentService::new();
        ...
        let result = service
            .apply_impairment(
                ...
                input.asset_id,
                input.adjustment_date,
                input.impairment_amount,
                input.reason,
                input.document_number,
                user_id,
            )
            .await
            .map_err(|e| format!("Failed to apply impairment: {}", e))?;
