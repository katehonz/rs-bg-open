...
impl CurrencyMutation {
    ...
    /// Ensure fixed EUR/BGN rate exists for a specific date
    async fn ensure_fixed_eur_bgn_rate(
        &self,
        ctx: &Context<'_>,
        date: NaiveDate,
    ) -> FieldResult<String> {
        ...
        fixed_service
            .ensure_eur_bgn_rate(db, date)
            .await
            .map_err(|e| format!("Failed to ensure EUR/BGN rate: {}", e))?;

        Ok(format!(
            "Fixed EUR/BGN rate ({}) ensured for {}",
            FixedRatesService::EUR_BGN_RATE,
            date
        ))
    }

    /// Ensure fixed EUR/BGN rates for today and recent dates
    async fn ensure_current_fixed_rates(&self, ctx: &Context<'_>) -> FieldResult<Vec<String>> {
        let db = ctx.data::<Arc<DatabaseConnection>>()?;
        let db = db.as_ref();
        let fixed_service = FixedRatesService::new();
        ...
        // Ensure fixed rate for today and last 30 days
        for days_ago in 0..30 {
            let date = today - chrono::Duration::days(days_ago);
            fixed_service
                .ensure_eur_bgn_rate(db, date)
                .await
                .map_err(|e| format!("Failed to ensure EUR/BGN rate for {}: {}", date, e))?;
