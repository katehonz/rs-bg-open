use crate::auth::{generate_token, AuthenticatedUser, JwtConfig};
...
impl UserMutation {
    ...
    async fn change_own_password(
        ...
        if new_password.len() < 6 {
            return Err("New password must be at least 6 characters long".into());
        }
        ...
        // Update password
        let mut user: user::ActiveModel = user::Entity::find_by_id(current_user.id)
            .one(db)
            .await?
    ...
    /// Generate recovery code for the current user
    async fn generate_recovery_code(
        ...
        if recovery_code.len() != 9 || !recovery_code.contains('-') {
            return Err("Recovery code must be in format XXXX-XXXX".into());
        }

        // Hash the recovery code
        let code_hash = user::Model::hash_password(&recovery_code)
        ...
        user.recovery_code_hash = Set(Some(code_hash));
        user.recovery_code_created_at = Set(Some(chrono::Utc::now()));
        user::Entity::update(user).exec(db).await?;
