...
impl AccountingMutation {
    ...
    async fn post_journal_entry(
        ...
        // Process inventory movements for lines with quantities
        let inventory_service = crate::services::inventory_service::InventoryService::new();
        for line in &lines {
            if line.quantity.is_some() && line.quantity.unwrap() > Decimal::ZERO {
                // Process the entry line to create inventory movement
                match inventory_service.process_entry_line(db, line.id).await {
                    Ok(_) => {},
                    Err(e) => {
                        eprintln!("Warning: Failed to process inventory movement for line {}: {}", line.id, e);
                        // Don't fail the entire post operation, just log the warning
                    }
                }
            }
        }
    ...
    async fn update_journal_entry(
        ...
        if let Some(document_type) = input.document_type {
            entry_model.document_type = Set(Some(document_type));
        }

        // Update VAT fields
        if let Some(vat_document_type) = input.vat_document_type {
            entry_model.vat_document_type = Set(Some(vat_document_type));
        }
        if let Some(vat_purchase_operation) = input.vat_purchase_operation {
            entry_model.vat_purchase_operation = Set(Some(vat_purchase_operation));
        }
        if let Some(vat_sales_operation) = input.vat_sales_operation {
            entry_model.vat_sales_operation = Set(Some(vat_sales_operation));
        }
        if let Some(vat_additional_operation) = input.vat_additional_operation {
            entry_model.vat_additional_operation = Set(Some(vat_additional_operation));
        }
        if let Some(vat_additional_data) = input.vat_additional_data {
            entry_model.vat_additional_data = Set(Some(vat_additional_data));
        }
        if let Some(vat_rate) = input.vat_rate {
            entry_model.vat_rate = Set(Some(vat_rate));
        }
    ...
    async fn delete_account(&self, ctx: &Context<'_>, id: i32) -> FieldResult<bool> {
        let db = ctx.data::<Arc<DatabaseConnection>>()?;
        let db = db.as_ref();
        ...
        // Check if there are any journal entry lines using this account
        let entry_lines_count = entry_line::Entity::find()
            .filter(entry_line::Column::AccountId.eq(id))
            .count(db)
            .await?;

        if entry_lines_count > 0 {
            return Err(format!(
                "Cannot delete account '{}' - {} journal entry lines exist. Delete those entries first or mark the account as inactive.",
                account.code,
                entry_lines_count
            ).into());
        }
        ...
        // Delete the account
        account::Entity::delete_by_id(id).exec(db).await?;

        Ok(true)
    }

    /// Delete multiple accounts (only if no journal entries exist)
    async fn delete_accounts(&self, ctx: &Context<'_>, ids: Vec<i32>) -> FieldResult<i32> {
        let db = ctx.data::<Arc<DatabaseConnection>>()?;
        let db = db.as_ref();

        // Check each account for journal entries
        for id in &ids {
            let account = account::Entity::find_by_id(*id)
                .one(db)
                .await?
        ...
        // Delete all accounts
        let delete_result = account::Entity::delete_many()
