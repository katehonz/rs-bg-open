//! GraphQL Resolvers for Inventory Management

use async_graphql::{Context, FieldResult, InputObject, Object, SimpleObject};
use chrono::NaiveDate;
use rust_decimal::Decimal;
use sea_orm::DatabaseConnection;
use std::sync::Arc;
use rust_xlsxwriter::*;
use base64::prelude::*;

use crate::services::inventory_service::{
    CorrectionNeeded as ServiceCorrectionNeeded, InventoryService, QuantityTurnoverRow,
};

#[derive(Default)]
pub struct InventoryQuery;

#[derive(SimpleObject)]
pub struct AverageCostResult {
    pub account_id: i32,
    pub current_quantity: Decimal,
    pub current_amount: Decimal,
    pub average_cost: Decimal,
}

#[derive(SimpleObject)]
pub struct QuantityTurnover {
    pub account_id: i32,
    pub account_code: String,
    pub account_name: String,
    pub opening_quantity: Decimal,
    pub opening_amount: Decimal,
    pub receipt_quantity: Decimal,
    pub receipt_amount: Decimal,
    pub issue_quantity: Decimal,
    pub issue_amount: Decimal,
    pub closing_quantity: Decimal,
    pub closing_amount: Decimal,
}

#[derive(SimpleObject)]
pub struct CorrectionNeeded {
    pub movement_id: i32,
    pub movement_date: NaiveDate,
    pub material_account_id: i32,
    pub expense_account_id: i32,
    pub quantity: Decimal,
    pub old_average_cost: Decimal,
    pub new_average_cost: Decimal,
    pub correction_amount: Decimal,
    pub description: String,
}

impl From<QuantityTurnoverRow> for QuantityTurnover {
    fn from(row: QuantityTurnoverRow) -> Self {
        Self {
            account_id: row.account_id,
            account_code: row.account_code,
            account_name: row.account_name,
            opening_quantity: row.opening_quantity,
            opening_amount: row.opening_amount,
            receipt_quantity: row.receipt_quantity,
            receipt_amount: row.receipt_amount,
            issue_quantity: row.issue_quantity,
            issue_amount: row.issue_amount,
            closing_quantity: row.closing_quantity,
            closing_amount: row.closing_amount,
        }
    }
}

impl From<ServiceCorrectionNeeded> for CorrectionNeeded {
    fn from(correction: ServiceCorrectionNeeded) -> Self {
        Self {
            movement_id: correction.movement_id,
            movement_date: correction.movement_date,
            material_account_id: correction.material_account_id,
            expense_account_id: correction.expense_account_id,
            quantity: correction.quantity,
            old_average_cost: correction.old_average_cost,
            new_average_cost: correction.new_average_cost,
            correction_amount: correction.correction_amount,
            description: correction.description,
        }
    }
}

#[Object]
impl InventoryQuery {
    /// Get average cost for a material account
    async fn get_average_cost(
        &self,
        ctx: &Context<'_>,
        company_id: i32,
        account_id: i32,
        as_of_date: Option<NaiveDate>,
    ) -> FieldResult<AverageCostResult> {
        let db = ctx.data::<Arc<DatabaseConnection>>()?;
        let service = InventoryService::new();

        let info = service
            .get_average_cost(db.as_ref(), company_id, account_id, as_of_date)
            .await?;

        Ok(AverageCostResult {
            account_id: info.account_id,
            current_quantity: info.current_quantity,
            current_amount: info.current_amount,
            average_cost: info.average_cost,
        })
    }

    /// Get quantity turnover statement
    async fn get_quantity_turnover(
        &self,
        ctx: &Context<'_>,
        company_id: i32,
        from_date: NaiveDate,
        to_date: NaiveDate,
    ) -> FieldResult<Vec<QuantityTurnover>> {
        let db = ctx.data::<Arc<DatabaseConnection>>()?;
        let service = InventoryService::new();

        let rows = service
            .get_quantity_turnover(db.as_ref(), company_id, from_date, to_date)
            .await?;

        Ok(rows.into_iter().map(QuantityTurnover::from).collect())
    }

    /// Check for needed corrections when adding retroactive entry
    async fn check_retroactive_corrections(
        &self,
        ctx: &Context<'_>,
        company_id: i32,
        account_id: i32,
        new_entry_date: NaiveDate,
    ) -> FieldResult<Vec<CorrectionNeeded>> {
        let db = ctx.data::<Arc<DatabaseConnection>>()?;
        let service = InventoryService::new();

        let corrections = service
            .check_retroactive_corrections(db.as_ref(), company_id, account_id, new_entry_date)
            .await?;

        Ok(corrections.into_iter().map(CorrectionNeeded::from).collect())
    }
}

#[derive(Default)]
pub struct InventoryMutation;

#[derive(InputObject)]
pub struct ProcessEntryLineInput {
    pub entry_line_id: i32,
}

#[derive(SimpleObject)]
pub struct ProcessEntryLineResult {
    pub success: bool,
    pub movement_id: Option<i32>,
    pub message: String,
}

#[derive(SimpleObject)]
pub struct InventoryExport {
    pub format: String,
    pub content: String, // base64 encoded
    pub filename: String,
    pub mime_type: String,
}

#[Object]
impl InventoryMutation {
    /// Process entry line to create inventory movement
    async fn process_entry_line(
        &self,
        ctx: &Context<'_>,
        input: ProcessEntryLineInput,
    ) -> FieldResult<ProcessEntryLineResult> {
        let db = ctx.data::<Arc<DatabaseConnection>>()?;
        let service = InventoryService::new();

        match service
            .process_entry_line(db.as_ref(), input.entry_line_id)
            .await
        {
            Ok(Some(movement)) => Ok(ProcessEntryLineResult {
                success: true,
                movement_id: Some(movement.id),
                message: "Inventory movement created successfully".to_string(),
            }),
            Ok(None) => Ok(ProcessEntryLineResult {
                success: true,
                movement_id: None,
                message: "No movement created (no quantity or already processed)".to_string(),
            }),
            Err(e) => Ok(ProcessEntryLineResult {
                success: false,
                movement_id: None,
                message: format!("Error: {}", e),
            }),
        }
    }

    /// Export quantity turnover report
    async fn export_quantity_turnover(
        &self,
        ctx: &Context<'_>,
        company_id: i32,
        from_date: NaiveDate,
        to_date: NaiveDate,
        format: String, // "XLSX"
    ) -> FieldResult<InventoryExport> {
        let query = InventoryQuery::default();
        let turnover_data = query
            .get_quantity_turnover(ctx, company_id, from_date, to_date)
            .await?;

        let filename = format!(
            "quantity_turnover_{}_{}.{}",
            from_date,
            to_date,
            format.to_lowercase()
        );

        match format.to_uppercase().as_str() {
            "XLSX" => {
                let content = generate_xlsx_quantity_turnover(&turnover_data, from_date, to_date, company_id)?;
                let encoded_content = base64::prelude::BASE64_STANDARD.encode(&content);
                Ok(InventoryExport {
                    format: "XLSX".to_string(),
                    content: encoded_content,
                    filename,
                    mime_type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                        .to_string(),
                })
            }
            _ => Err("Unsupported format".into()),
        }
    }
}

/// Generate XLSX for quantity turnover
fn generate_xlsx_quantity_turnover(
    data: &[QuantityTurnover],
    from_date: NaiveDate,
    to_date: NaiveDate,
    _company_id: i32,
) -> FieldResult<Vec<u8>> {
    let mut workbook = Workbook::new();
    let worksheet = workbook.add_worksheet();

    // Set column widths
    worksheet.set_column_width(0, 15)?; // Account ID
    worksheet.set_column_width(1, 12)?; // Opening Qty
    worksheet.set_column_width(2, 14)?; // Opening Amount
    worksheet.set_column_width(3, 12)?; // Receipt Qty
    worksheet.set_column_width(4, 14)?; // Receipt Amount
    worksheet.set_column_width(5, 12)?; // Issue Qty
    worksheet.set_column_width(6, 14)?; // Issue Amount
    worksheet.set_column_width(7, 12)?; // Closing Qty
    worksheet.set_column_width(8, 14)?; // Closing Amount

    // Title row
    let title_format = Format::new()
        .set_bold()
        .set_font_size(14);

    worksheet.merge_range(0, 0, 0, 8, "Количествена оборотна ведомост", &title_format)?;

    // Period row
    let period_format = Format::new().set_font_size(10);
    worksheet.merge_range(
        1, 0, 1, 8,
        &format!("Период: {} - {}", from_date, to_date),
        &period_format
    )?;

    // Header format
    let header_format = Format::new()
        .set_bold()
        .set_align(FormatAlign::Center)
        .set_background_color(Color::RGB(0xD3D3D3))
        .set_border(FormatBorder::Thin);

    // Main headers (row 3)
    worksheet.write_string_with_format(3, 0, "Сметка", &header_format)?;
    worksheet.merge_range(3, 1, 3, 2, "Начално салдо", &header_format)?;
    worksheet.merge_range(3, 3, 3, 4, "Постъпване (Дт)", &header_format)?;
    worksheet.merge_range(3, 5, 3, 6, "Разход (Кт)", &header_format)?;
    worksheet.merge_range(3, 7, 3, 8, "Крайно салдо", &header_format)?;

    // Sub-headers (row 4)
    worksheet.write_string_with_format(4, 1, "Кол-во", &header_format)?;
    worksheet.write_string_with_format(4, 2, "Стойност", &header_format)?;
    worksheet.write_string_with_format(4, 3, "Кол-во", &header_format)?;
    worksheet.write_string_with_format(4, 4, "Стойност", &header_format)?;
    worksheet.write_string_with_format(4, 5, "Кол-во", &header_format)?;
    worksheet.write_string_with_format(4, 6, "Стойност", &header_format)?;
    worksheet.write_string_with_format(4, 7, "Кол-во", &header_format)?;
    worksheet.write_string_with_format(4, 8, "Стойност", &header_format)?;

    // Data format
    let number_format = Format::new()
        .set_num_format("#,##0.00")
        .set_border(FormatBorder::Thin);

    let currency_format = Format::new()
        .set_num_format("#,##0.00 лв")
        .set_border(FormatBorder::Thin);

    let text_format = Format::new()
        .set_border(FormatBorder::Thin);

    // Data rows
    let mut row = 5;
    for item in data {
        worksheet.write_string_with_format(row, 0, &format!("{} - {}", item.account_code, item.account_name), &text_format)?;
        worksheet.write_number_with_format(row, 1, item.opening_quantity.to_string().parse::<f64>().unwrap_or(0.0), &number_format)?;
        worksheet.write_number_with_format(row, 2, item.opening_amount.to_string().parse::<f64>().unwrap_or(0.0), &currency_format)?;
        worksheet.write_number_with_format(row, 3, item.receipt_quantity.to_string().parse::<f64>().unwrap_or(0.0), &number_format)?;
        worksheet.write_number_with_format(row, 4, item.receipt_amount.to_string().parse::<f64>().unwrap_or(0.0), &currency_format)?;
        worksheet.write_number_with_format(row, 5, item.issue_quantity.to_string().parse::<f64>().unwrap_or(0.0), &number_format)?;
        worksheet.write_number_with_format(row, 6, item.issue_amount.to_string().parse::<f64>().unwrap_or(0.0), &currency_format)?;
        worksheet.write_number_with_format(row, 7, item.closing_quantity.to_string().parse::<f64>().unwrap_or(0.0), &number_format)?;
        worksheet.write_number_with_format(row, 8, item.closing_amount.to_string().parse::<f64>().unwrap_or(0.0), &currency_format)?;
        row += 1;
    }

    let buffer = workbook.save_to_buffer()?;
    Ok(buffer)
}
