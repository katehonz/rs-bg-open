...
use crate::entities::{
    company::{ActiveModel as CompanyActiveModel, Entity as CompanyEntity, Model as CompanyModel, RateProvider},
...
use crate::graphql::context::get_current_user;
use crate::auth::{user_can_access_company, user_is_company_admin};
...
impl AdminQuery {
    ...
    async fn companies(&self, ctx: &Context<'_>) -> FieldResult<Vec<CompanyModel>> {
        ...
        // Fetch user's group
        let user_group = UserGroupEntity::find_by_id(current_user.group_id)
            .one(db)
            .await?
            .ok_or_else(|| "User group not found")?;

        // Superadmins and admins can see all companies
        ...
        // Other users can only see companies they are assigned to
        let user_companies = UserCompanyEntity::find()
            .filter(crate::entities::user_company::Column::UserId.eq(current_user.id))
            .filter(crate::entities::user_company::Column::IsActive.eq(true))
            .all(db)
            .await?;

        let company_ids: Vec<i32> = user_companies.iter().map(|uc| uc.company_id).collect();
        ...
        let companies = CompanyEntity::find()
            .filter(crate::entities::company::Column::Id.is_in(company_ids))
            .order_by_asc(crate::entities::company::Column::Name)
            .all(db)
            .await?;
    ...
    async fn company(&self, ctx: &Context<'_>, id: i32) -> FieldResult<Option<CompanyModel>> {
        ...
        user_can_access_company(current_user.id, id, db).await
    ...
    async fn user_companies(&self, ctx: &Context<'_>) -> FieldResult<Vec<UserCompanyWithDetails>> {
        ...
        // Get companies that current user has access to
        let current_user_companies = UserCompanyEntity::find()
            .filter(crate::entities::user_company::Column::UserId.eq(current_user.id))
            .filter(crate::entities::user_company::Column::IsActive.eq(true))
            .all(db)
            .await?;

        let accessible_company_ids: Vec<i32> = current_user_companies.iter().map(|uc| uc.company_id).collect();

        // Only return user-company relationships for companies current user has access to
        let user_companies = UserCompanyEntity::find()
            .filter(crate::entities::user_company::Column::CompanyId.is_in(accessible_company_ids))
    ...
    async fn users_by_company(
        ...
        user_can_access_company(current_user.id, company_id, db).await
            .map_err(|e| async_graphql::Error::new(e))?;
...
impl AdminMutation {
    ...
    async fn admin_delete_user(&self, ctx: &Context<'_>, id: i32) -> FieldResult<bool> {
        let db = ctx.data::<DatabaseConnection>()?;
    ...
    async fn update_company(
        ...
        if let Some(base_currency_id) = input.base_currency_id {
            company.base_currency_id = Set(Some(base_currency_id));
        }
        if let Some(preferred_rate_provider) = input.preferred_rate_provider {
            company.preferred_rate_provider = Set(preferred_rate_provider);
        }
    ...
    async fn assign_user_to_company(
        ...
        let is_admin = user_is_company_admin(current_user.id, input.company_id, db).await
            .map_err(|e| async_graphql::Error::new(format!("Authorization check failed: {}", e)))?;

        if !is_admin {
            return Err(async_graphql::Error::new("Permission denied: must be company admin to assign users"));
        }
    ...
    async fn update_user_company(
        ...
        // Check if current user is admin of the company
        let is_admin = user_is_company_admin(current_user.id, user_company.company_id, db).await
            .map_err(|e| async_graphql::Error::new(format!("Authorization check failed: {}", e)))?;

        if !is_admin {
            return Err(async_graphql::Error::new("Permission denied: must be company admin to update user roles"));
        }
    ...
    async fn remove_user_from_company(&self, ctx: &Context<'_>, id: i32) -> FieldResult<bool> {
        ...
        // First, get the user_company to find the company_id
        let user_company = UserCompanyEntity::find_by_id(id)
            .one(db)
            .await?
            .ok_or("User-Company relationship not found")?;
        ...
        if !is_admin {
            return Err(async_graphql::Error::new("Permission denied: must be company admin to remove users"));
        }
