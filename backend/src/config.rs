...
impl Config {
    ...
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        let database_url = std::env::var("DATABASE_URL")
            .unwrap_or_else(|_| {
                eprintln!("WARNING: Using default database credentials! Set DATABASE_URL environment variable in production!");
                "postgres://postgres:CHANGE_ME_IN_PRODUCTION@localhost/rs_ac_bg".to_string()
            });
        ...
        let (db_host, db_port, db_user, db_pass, db_name) = Self::parse_database_url(&database_url)
            .unwrap_or_else(|| ("localhost".to_string(), 5432, "postgres".to_string(), "CHANGE_ME_IN_PRODUCTION".to_string(), "rs_ac_bg".to_string()));
