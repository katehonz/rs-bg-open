...
pub struct Model {
    ...
    // Document type for journal entries (INVOICE, PROTOCOL, CASH_ORDER, etc.)
    ...
    fn from(input: CreateJournalEntryInput) -> Self {
        ActiveModel {
            entry_number: Set(input.entry_number.unwrap_or_else(|| {
                // Generate unique entry number with microseconds to avoid collisions
                let now = chrono::Utc::now();
                format!("JE-{}-{}", now.format("%Y%m%d-%H%M%S"), now.timestamp_subsec_micros())
            })),
