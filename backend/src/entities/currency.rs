...
impl Model {
    ...
    pub fn format_amount(&self, amount: rust_decimal::Decimal) -> String {
        let symbol = self.symbol.as_deref().unwrap_or(&self.code);
        match self.decimal_places {
            0 => format!("{} {}", amount.round(), symbol),
            1 => format!("{:.1} {}", amount, symbol),
            2 => format!("{:.2} {}", amount, symbol),
            3 => format!("{:.3} {}", amount, symbol),
            4 => format!("{:.4} {}", amount, symbol),
            _ => format!("{:.6} {}", amount, symbol),
        }
    }
    ...
    /// Get common currency pairs for Bulgaria
    pub fn bulgarian_common_currencies() -> Vec<(&'static str, &'static str, &'static str)> {
        vec![
