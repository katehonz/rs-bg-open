...
impl MigrationTrait for Migration {
        ...
        if count == 0 {
            ...
            let _insert_result = db
                .execute(Statement::from_string(
                    ...
                    VALUES ('Demo Company Ltd', 'BG000000000', 'Demo Address',
                            '+000 00 000 0000', 'demo@example.com', 'Demo User', true, '000000000')
                    RETURNING id
                    "#.to_string(),
                ))
                .await?;
            ...
            let company_id_result = db
                .query_one(Statement::from_string(
                    manager.get_database_backend(),
                    "SELECT id FROM companies WHERE name = 'Demo Company Ltd'".to_string(),
                ))
                .await?;
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        db.execute(Statement::from_string(
            manager.get_database_backend(),
            "DELETE FROM companies WHERE name = 'Demo Company Ltd'".to_string(),
        ))
        .await?;
